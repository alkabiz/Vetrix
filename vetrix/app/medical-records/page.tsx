...
export default function MedicalRecordsPage() {
  ...
  const handleEditRecord = (record: MedicalRecord) => {
    if (!user || !["admin", "vet"].includes(user.role)) {
      toast({
        title: "Access Denied",
        description: "Only veterinarians and administrators can edit medical records",
        variant: "destructive",
      })
      return
    }
  ...
  const handleDeleteRecord = async (record: MedicalRecord) => {
    if (!user || !["admin", "vet"].includes(user.role)) {
      toast({
        title: "Access Denied",
        description: "Only veterinarians and administrators can delete medical records",
        variant: "destructive",
      })
      return
    }

    if (!confirm(`Are you sure you want to delete this medical record for ${record.pet_name}?`)) {
      return
    }
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              {user?.role === "assistant"
                ? "View pet consultations, diagnoses, and treatments (read-only access)"
                : "Track pet consultations, diagnoses, and treatments"}
            </p>
          </div>
          ...
          {user?.role === "assistant" && (
            <Card className="bg-gradient-to-r from-amber-50 to-yellow-50 border-amber-200">
              <CardHeader>
                <CardTitle className="text-amber-900">Assistant Access</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-amber-700 text-sm">
                  You have read-only access to medical records. Only veterinarians and administrators can create, edit,
                  or delete medical records.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Unique Pets</CardTitle>
                <Stethoscope className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{stats.uniquePets}</div>
                <p className="text-xs text-muted-foreground">Pets with records</p>
              </CardContent>
            </Card>
          </div>
          ...
          <DataTable
            ...
            description={`${filteredRecords.length} medical record${filteredRecords.length !== 1 ? "s" : ""} ${petFilter !== "all" ? `for selected pet` : ""}`}
            ...
            addButtonText="Add Medical Record"
          />
