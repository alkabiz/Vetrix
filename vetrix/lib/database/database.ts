...
export function getDatabase() {
  if (!db) {
    const dbPath = join(process.cwd(), "vetrix.db")
...
export interface User {
  ...
  passwordHistory?: string[] | null // JSON array of hashes

  // Autenticación de dos factores
  twoFactorEnabled: boolean
  twoFactorSecret?: string | null
  backupCodes?: string[] | null // JSON array of codes
...
export const sql = {
  ...
  query: (query: string, params?: any[]) => {
    console.log("[v0] Mock SQL query:", query, params)
    return []
  },
}