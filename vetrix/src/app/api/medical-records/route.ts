...
export const GET = requireAnyRole(async (request: AuthenticatedRequest) => {
  try {
    ...
    return NextResponse.json({ error: "No se pudieron obtener los registros médicos" }, { status: 500 })
  }
})
...
export const POST = requireVetOrAdmin(async (request: AuthenticatedRequest) => {
  try {
    ...
    console.error("Error al crear el expediente médico:", error)
    return NextResponse.json({ error: "No se pudo crear el expediente médico" }, { status: 500 })
  }
})
